...
admission_2021 <- admission_2021 |>
  filter(...4 == '초등학교', ...3 != '소계') |>
  select(1, 2, 3, 4, 92, 93) |>
  rename('연도' = ...1, '시도' = ...2, '행정구역' = ...3, '학제' = ...4, '입학자_계' = ...92, '입학자_여' = ...93) |>
  mutate(입학자_남 = 입학자_계 - 입학자_여)

## 로딩된 데이터 중 15~20년 초등학교 1학년 데이터만 필터링
admission_15_20 <- admission_15_20 |>
  filter(...4 == '초등학교', ...3 != '소계') |>
  select(1, 2, 3, 4, 67, 68) |>
  rename('연도' = ...1, '시도' = ...2, '행정구역' = ...3, '학제' = ...4, '입학자_계' = ...67, '입학자_여' = ...68) |>
  mutate(입학자_남 = 입학자_계 - 입학자_여)
...
result_입학생 <- left_join(join_result, total_pop, by = c('연도', '시도', '행정구역'))

write.csv(result_입학생, '입학생.csv', sep = '\t', fileEncoding = 'cp949')
...
fig <- admission_model_grid |> filter(성별 == '전체', is.na(code) != T) |>
  ...
  geom_line(aes(group = code), color = 'white') + 
  geom_line(aes(y = n_2_diff_rate, group = code), color = 'grey50') + 
  #  geom_point(aes(size = `4세대비1학년변동치`), color = 'white', shape = 1) + 
  geom_text(aes(x = 5, y = 0.7, label = paste(시도, 행정구역)), size = 2, hjust = 0.5, color = 'black') +
